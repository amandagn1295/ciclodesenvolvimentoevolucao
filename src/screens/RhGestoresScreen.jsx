...
(function () {
  ...
  function RhGestoresScreen({ gestores, funcionarios, ciclos, catalogos, onAddCatalogoItem, onAdd, onUpdate, onDelete, onImportBulk, onToast }) {
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
        ...
        <Card title="Gestores Cadastrados" action={<Button size="sm" variant="secondary" iconLeft={<Icon name="file-down" size={14} />} onClick={() => window.CicloBulk.exportRows(list, [
          { label: "Nome", key: "nome" }, { label: "Cargo", key: "cargo" }, { label: "Diretoria", key: "diretoria" }, { label: "Departamento", key: "departamento" },
          { label: "Setor", key: "setor" }, { label: "Unidade", key: "unidade" }, { label: "Regional", key: "regional" },
          { label: "Usuário", key: "usuario" }, { label: "Colaboradores", value: (r) => qtdColabs(r.id) }, { label: "Status", value: (r) => (r.desligado ? "Desligado" : "Ativo") },
        ], "gestores.xlsx", "Gestores")}>Exportar (Excel)</Button>}>
              ...
              <div key={g.id} style={{ border: "1px solid var(--border-subtle)", borderRadius: "var(--radius-lg)", padding: "14px 16px", display: "flex", gap: 14, alignItems: "flex-start", opacity: g.desligado ? 0.72 : 1 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                    <span style={{ fontWeight: 700, fontSize: 14, cursor: "pointer", color: "var(--ab-azul-escuro)", textDecoration: "underline dotted" }} onClick={() => setHistId(g.id)}>{g.nome}</span>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  ...
                  <Button size="sm" variant="ghost" onClick={() => setHistId(g.id)} iconLeft={<Icon name="history" size={14} />}>Histórico</Button>
        ...
        {gestores.filter((g) => g.id === histId).map((g) => (
          <window.GestorHistoricoView key={g.id} gestor={g} ciclosDoGestor={(ciclos || []).filter((c) => c.funcionarioGestorId === g.id)} funcionariosDoGestor={(funcionarios || []).filter((f) => f.gestorId === g.id)} onClose={() => setHistId(null)} />
        ))}
      </div>
    );
  }
