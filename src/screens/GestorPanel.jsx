...
(function () {
  ...
  function GestorPanel({
    gestor, logoSrc, funcionarios, ciclosView, auditLog, todosGestores, ciclosViewGeral, todosFuncionarios,
    onAddFuncionario, onUpdateFuncionario, onDeleteFuncionario,
    onAddCiclo, onDeleteCiclo, onUpdateAction, onSaveAvaliacaoForm, onSaveFeedbackForm, onSubmitColabForm,
    ...
    if (colabFormFor) {
      ...
      return (<>
        <window.FormColaboradorScreen colab={live} historico={historicoColab} logoSrc={logoSrc} readOnly={live.encerrado} onToast={toast} onBack={() => setColabFormFor(null)} onUpdateAction={onUpdateAction} onSubmit={(form) => onSubmitColabForm(live.id, form)} />
        {toastNode}
        {renderTweaks()}
      </>);
    }
      ...
      <div style={{ minHeight: "100vh", background: "var(--surface-canvas)" }}>
        <AppHeader
          ...
          tabs={[
            ...
            { id: "avaliacao", label: "Avaliação do Gestor", icon: <Icon name="clipboard-check" size={15} /> },
            { id: "feedback", label: "Reunião de Feedback", icon: <Icon name="handshake" size={15} /> },
          ]}
          right={<>
        ...
        <main style={{ maxWidth: "var(--container-max)", margin: "0 auto", padding: "24px" }}>
          {view === "dashboard" && <window.DashboardScreen colabs={ciclosView} funcionarios={funcionarios} onOpen={setModalColab} gestor={gestor} todosGestores={todosGestores} ciclosViewGeral={ciclosViewGeral} todosFuncionarios={todosFuncionarios} onNavigate={setView} />}
          {view === "colaboradores" && <window.FuncionariosScreen funcionarios={funcionarios} ciclos={ciclosView} auditLog={auditLog} gestorNome={gestor.nome} onAdd={onAddFuncionario} onUpdate={onUpdateFuncionario} onDelete={onDeleteFuncionario} onToast={toast} />}
          ...
          {view === "avaliacao" && <window.FormAvaliacaoScreen colabs={ciclosView} onSave={onSaveAvaliacaoForm} onToast={toast} />}
          {view === "feedback" && <window.FormFeedbackScreen colabs={ciclosView} onSave={onSaveFeedbackForm} onToast={toast} />}
        </main>
