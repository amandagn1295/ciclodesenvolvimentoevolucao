    ...
    return (
      <Modal
        ...
        footer={<>
          ...
          {!colab.encerrado
            ? <Button variant="primary" onClick={() => onFinalize(colab.id)} iconLeft={<Icon name="flag" size={14} />}>Finalizar Ciclo</Button>
      ...
      >
        ...
        {g.rowsTec && g.rowsTec.length > 0 && (
          <div style={{ marginBottom: 18 }}>
            ...
            <PdiTable rows={g.rowsTec} readOnly />
          </div>
        )}
        {g.rowsComp && g.rowsComp.length > 0 && (
          <div style={{ marginBottom: 18 }}>
            ...
            <PdiTable rows={g.rowsComp} readOnly />
          </div>
        )}
        {g.conexao && (
          <Banner tone="info" icon={<Icon name="handshake" size={16} />} title="Conexão Líder">{g.conexao}</Banner>
        )}
      </Modal>
    );
  }
