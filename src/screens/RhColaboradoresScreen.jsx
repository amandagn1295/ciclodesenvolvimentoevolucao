...
(function () {
  ...
  const { Icon, fmtDate, tempoEmpresa, EditableSelect, parseDataBR } = window.CicloHelpers;

  const HEADERS = ["Nome", "Usuário do Gestor", "Cargo", "Departamento", "Diretoria | Negócios", "Setor", "Regional", "Admissão (DD/MM/AAAA)", "WhatsApp"];

  function RhColaboradoresScreen({ funcionarios, gestores, ciclos, catalogos, onAddCatalogoItem, onAdd, onUpdate, onDelete, onImportBulk, onToast }) {
    ...
    return (
      <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
        ...
        <Card title={`Todos os Colaboradores (${list.length})`}>
              ...
              <div key={c.id} style={{ border: "1px solid var(--border-subtle)", borderRadius: "var(--radius-lg)", padding: "14px 16px", display: "flex", gap: 14, alignItems: "flex-start", opacity: c.demitido ? 0.72 : 1 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                    ...
                    {ciclosDe(c.id) > 0 && <span style={{ fontSize: 11, color: "var(--ab-azul-escuro)", background: "var(--surface-brand-soft)", padding: "2px 8px", borderRadius: 999, fontWeight: 700 }}>{ciclosDe(c.id)} ciclo(s)</span>}
                  </div>
