...
(function () {
  ...
  function App() {
    ...
    const addCiclo = (dados) => {
      const novo = {
        id: genId(), acesso: genAcesso(), encerrado: false, gestorForm: null, colabForm: null,
    ...
    const finalize = (id) => {
      setCiclos((cs) => cs.map((c) => (c.id === id ? { ...c, encerrado: true } : c)));
    ...
    const updateAction = (cicloId, tipo, index, patch) => {
      ...
      setCiclos((cs) => cs.map((c) => {
        ...
        const rows = (c.gestorForm[key] || []).map((r, i) => (i === index ? { ...r, ...patch } : r));
        return { ...c, gestorForm: { ...c.gestorForm, [key]: rows } };
      }));
    };
    const saveGestorForm = (cicloId, gestorForm) => {
      setCiclos((cs) => cs.map((c) => (c.id === cicloId ? { ...c, gestorForm } : c)));
    ...
    if (publicColabId) {
      ...
      return (<>
        <window.FormColaboradorScreen colab={c} logoSrc={LOGO_BRANCA} readOnly={c.encerrado} onToast={toast} onBack={null} onUpdateAction={updateAction} onSubmit={(form) => submitColabForm(c.id, form)} />
        {toastNode}
      </>);
    }
    ...
    return (<>
      <window.GestorPanel
        ...
        funcionarios={meusFuncionarios} ciclosView={meusCiclosView}
        ...
        onSaveGestorForm={saveGestorForm} onSubmitColabForm={submitColabForm} onFinalize={finalize}
